...
def insertDoc1(userid="NA",title="NA",docPath="NA",docDesc='NA',dt="NA",tm="NA",key='NA',) : 
    ...
    cnt=cursor.rowcount 
    conn.commit()
def insertWDoc1(doc="NA",userid="NA") : 
    conn = connect()    
    cursor = conn.cursor()
    args = [userid,doc]
    args1=cursor.callproc('insertWDoc', args)
    print("Return value:", args1)
    #for result in cursor.stored_results():
     #       print(result.fetchall())
    cnt=cursor.rowcount 
    conn.commit()
def insertRecDetails(userid="NA",ruserid="NA",docId="NA",wlen=0,flen=0,fhash="NA") : 
    conn = connect()    
    cursor = conn.cursor()
    args = [docId,ruserid,userid,flen,wlen,fhash]
    args1=cursor.callproc('insertRecPermission', args)
    print("Return value:", args1)
    #for result in cursor.stored_results():
     #       print(result.fetchall())
    cnt=cursor.rowcount 
...
def convertToBase64FromBytes(message_bytes) :
    #message_bytes = message.encode('ascii')
    base64_bytes = base64.b64encode(message_bytes)
    base64_message = base64_bytes.decode('ascii')
    print(base64_message)
