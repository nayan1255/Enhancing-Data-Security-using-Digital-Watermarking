...
public class WaterMarkingController implements ErrorController {
	...
	public ModelAndView OTPAuth1(Documents eobj,HttpServletRequest request,HttpSession ses)
	{
		...
		String ruserid="";
		 ...
		 try {
			 ...
			 eobj.setUserid(ses.getAttribute("userid").toString().trim());
			 ruserid=ses.getAttribute("userid").toString().trim();
			 if(eobj.getOtp().equals(eobj.getUotp()))
			 {
				 System.out.println("otp verified");
				 Base64Encoder encoder=new  Base64Encoder();
				...
				// Base64Decoder decode=new Base64Decoder();
				 ...
				 Vector v=cf.getValue("select userid from getdocuments where docid="+request.getParameter("docId").toString().trim() , 1);
				 userid=v.elementAt(0).toString().trim();
				 String param= userid+"|"+ruserid+"|"+request.getParameter("filePath").toString().trim() ;
				 System.out.println("param="+param);
				 param=encoder.encode(param.getBytes());
				 mv.addObject("path", GetURL.getPythonServerURL()+"DecryptDoc1.py?param="+param);
	...
	@RequestMapping("/FromPythonDec")
   	public ModelAndView FromPythonDec(ServletRequest request)
   	{
   	 ...
   	 @RequestMapping("/FromPythonFileHash")
   	public ModelAndView FromPythonFileHash(ServletRequest request)
   	{
   		ModelAndView mv=new ModelAndView();  
   		 try {
   			...
   			String fhash=request.getParameter("fhash").toString().trim();
   			CloudFuns cf=new CloudFuns();
   			Vector v=cf.getValue("select wlen,flen,docid from recipients where ruserid='"+userid+"' and fhash='"+fhash+"'" , 3);
   			if(v.size()>0)
   			{
   				Vector vd=cf.getValue("select filepath,skey from getdocuments where docid="+v.elementAt(2).toString().trim() , 2);
   	   			
   			Base64Encoder encoder=new Base64Encoder();
			 mv.setViewName("gotoPython.jsp");
			 String seckey=vd.elementAt(1).toString().trim();
			 String fnm=vd.elementAt(0).toString().trim();
			 RC6 rc6=new RC6();
			 System.out.println("str="); 
			 String filepath=request.getServletContext().getRealPath("/")+"/Uploads/";
			 Path path= Paths.get(filepath+"/"+fnm.split("\\.")[0]+".txt");
			 System.out.println("str="+filepath+"/"+fnm.split("\\.")[0]+".txt");
			 byte[] b1=Files.readAllBytes(path);
			 byte[] b=rc6.decrypt(b1, seckey.trim().getBytes());
			 Base64Decoder decode=new Base64Decoder();
			 String str=new String(decode.decode(new String(b)));
			 System.out.println("str="+str);
			 String[] arr=str.split("\\,");
			 String param= String.valueOf(v.elementAt(0).toString().trim()+"|"+v.elementAt(1).toString().trim()+"|"+docid+"|"+fname+"|"+userid)+"|"+arr[2].trim()+"|"+arr[3].trim()+"|"+arr[4].trim()+"|"+arr[5].trim() ;
			 
			 //String param=String.valueOf(v.elementAt(0).toString().trim()+"|"+v.elementAt(1).toString().trim()+"|"+docid+"|"+fname+"|"+userid);
			 System.out.println("param="+param);
			 param=encoder.encode(param.getBytes());
			 mv.addObject("path",GetURL.getPythonServerURL()+"DecWaterMark.py?param="+param);
			 
   			}
   			else
   			{
   				Vector v1=cf.getValue("select userid,ruserid,docid from recipients where  fhash='"+fhash+"'" , 3);
   			 
	   			if(v1.size()>0)
	   			{
	   				Vector v2=cf.getValue("select ifnull(max(leakId),1000)+1 from leakageTrack", 1);
	   				String qr="insert into leakageTrack values("+v2.elementAt(0).toString().trim()+",'"+v1.elementAt(0).toString().trim()+"','"+v1.elementAt(1).toString().trim()+"',"+v1.elementAt(2).toString().trim()+")";
	   				if(cf.execute(qr))
	   				{}
	   			}
   			 mv.setViewName("Failure.jsp?type=leakage");
   			 mv.addObject("activity","leakage");
   			}
   		 }
   		 catch (Exception e) {
   			// TODO: handle exception
   	...
   	public ModelAndView FromPythonDec1(ServletRequest request)
   	{
   		ModelAndView mv=new ModelAndView();  
   		 try {
				...
				CloudFuns cf=new CloudFuns();
				cf.recordusage(request.getParameter("ruserid").toString().trim(), "watermarking");
			//Vector v=cf.getValue("select doc from tempdoc where userid='"+request.getParameter("ruserid").toString().trim()+"'", 1)	 ;
			//String doc=v.elementAt(0).toString().trim();
			// Base64Decoder decode=new Base64Decoder();
			// byte[] b=decode.decode(doc);
			// String filepath=request.getServletContext().getRealPath("/")+"/Uploads/temp/";
   			 
			// FileOutputStream fos = new FileOutputStream(new File(filepath+request.getParameter("sts").toString().trim())); 
			//    fos.write(b); 
			 //   fos.close();
	...
	@RequestMapping("/FromPythonDec2")
   	public ModelAndView FromPythonDec2(ServletRequest request)
   	{
   		ModelAndView mv=new ModelAndView();  
   		 try {
				...
				CloudFuns cf=new CloudFuns();
				cf.recordusage(request.getParameter("userid").toString().trim(), "decryption");
				 
   			 mv.setViewName("download3.jsp");
   				 mv.addObject("sts",st);
   		 }
   		 catch (Exception e) {
   			// TODO: handle exception
	...
	@RequestMapping("/viewLeakageReport")
	@SessionScope
	public ModelAndView viewLeakageReport(HttpSession ses) {
		ModelAndView mv=new ModelAndView();
		try
		{
		List<models.Leakage> lst = new ArrayList<Leakage>();
		//CallMinnerAPI vs = new CallMinnerAPI();
		...
		mv.addObject("std",obj.getLst());
		mv.setViewName("ViewLeakageReport.jsp");
		}
		catch (Exception e) {
			// TODO: handle exception
			System.out.println("err="+e.getMessage());
	...
	public String RegUser(UserReg obj)
	{
		 ...
		 {
			 CloudFuns cf=new CloudFuns();
			 Vector v=cf.getValue("select mobile from userdetails where mobile='"+obj.getMobile()+"'", 1);
			 if(v.size()>0)
			 {
				 return "Failure.jsp?type=MobileNoRepeat";
			 }
			 else
			 { 
			 if(obj.registration() )
